...
首页背景由 `src/components/LandingExperience.astro` 统一管理，候选项为当前图片、工作室视频、60fps 单色粒子曲面视频和现代建筑光影视频。

- 每次访问都使用保存在 `localStorage` 中的随机袋选择背景，四种背景均匀出现且不会连续重复；无法使用本地存储时仍会即时随机。
...
- 固定预览入口为 `/video-preview/`、`/video-preview-2/` 和 `/video-preview-3/`，三个视频方案均已加入首页随机池。所有方案共用同一内容坐标，仅分别调整背景媒体的裁切与焦点。
